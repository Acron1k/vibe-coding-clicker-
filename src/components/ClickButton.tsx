  ...
  const onClick = useCallback((e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (rect) {
      ...
      setFloatingNumbers((prev) => [
        ...prev,
        { id: newId, value: earnedValue, x, y, isCrit },
      ])
        ...
        {floatingNumbers.map((num) => (
          <motion.div
            ...
            className={`absolute pointer-events-none font-bold font-mono z-20 ${
              num.isCrit ? 'text-neon-yellow text-3xl' : 'text-neon-green text-xl'
            }`}
            style={{ left: `calc(50% + ${num.x - 112}px)`, top: `calc(50% + ${num.y - 112}px)` }}
            initial={{ opacity: 1, y: 0, scale: num.isCrit ? 1.5 : 1 }}
            animate={{ opacity: 0, y: -80, scale: num.isCrit ? 2 : 1.2 }}
            exit={{ opacity: 0 }}
